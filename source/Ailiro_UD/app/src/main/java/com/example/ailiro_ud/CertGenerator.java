...
public class CertGenerator {
    ...
    public static X509Certificate generateCert(KeyPair keyPair) throws Exception {
        X500Name issuer = new X500Name("CN=Test Certificate, O=MyOrg, C=CN");
    ...
    public static PublicKey decodeECPublicKey(byte[] pubBytes, String curveName) throws Exception {
        ...
        // 2) 用 BC 解点
        org.bouncycastle.math.ec.ECPoint bcPoint = x9.getCurve().decodePoint(pubBytes);

        // 3) 将 BC 点转换成 JDK ECPoint（关键步骤）
        java.security.spec.ECPoint javaPoint = new java.security.spec.ECPoint(
                bcPoint.getXCoord().toBigInteger(),
                bcPoint.getYCoord().toBigInteger()
        );

        // 4) 将 BC 曲线参数转为 JDK 格式
        EllipticCurve curve = new EllipticCurve(
                new ECFieldFp(x9.getCurve().getField().getCharacteristic()),
                x9.getCurve().getA().toBigInteger(),
                x9.getCurve().getB().toBigInteger()
        );

        ECParameterSpec params = new ECParameterSpec(
                curve,
                new java.security.spec.ECPoint(
                        x9.getG().getXCoord().toBigInteger(),
                        x9.getG().getYCoord().toBigInteger()
                ),
                x9.getN(),
                x9.getH().intValue()
        );

        // 5) 最终构造 JDK KeySpec（而非 BC KeySpec）
        ECPublicKeySpec keySpec = new ECPublicKeySpec(javaPoint, params);

        // 6) 使用 Android 自带的 Provider，而不是 "BC"
        KeyFactory kf = KeyFactory.getInstance("EC");
...
//    }
    ...
    public static PublicKey loadPQCPublicKeyFromCertRaw(
            Context context,
            int rawResId
    ) throws Exception {
        ...
        try (InputStream is =
                     context.getResources().openRawResource(rawResId);
            ...
            if (!(obj instanceof X509CertificateHolder)) {
                throw new IllegalArgumentException(
                        "PEM does not contain X509 certificate"
                );
            }
        ...
        // 自签名校验
        cert.verify(cert.getPublicKey(), "BCPQC");
...
//    }
    ...
    public static Map<String, byte[]> parseProfile0000(byte[] der) throws Exception {
        ...
        for (int i = 0; i < dataSeq.size(); i++) {
            ...
            switch (tag) {
                ...
                case 7: out.put("extension", val); break;
    ...
    public static X509Certificate decompressToX509Certificate(Map<String, byte[]> profileMap) throws Exception {
        ...
        //BC标准 Dilithium OID（由 PQCConfig.DILITHIUM_LEVEL 决定）
        ASN1ObjectIdentifier DILITHIUM_OID = PQCConfig.dilithiumOid();
        ...
        byte[] extension = profileMap.get("extension");
        ...
        X500NameBuilder inameBuilder = new X500NameBuilder(BCStyle.INSTANCE);
        inameBuilder.addRDN(BCStyle.CN, "Issuer");
        inameBuilder.addRDN(BCStyle.O, "MyOrg");
        inameBuilder.addRDN(BCStyle.C, "CN");
        X500Name issuerName = inameBuilder.build();
//        X500Name subjectName = buildX500NameFromBytesOrCN(subjectBytes, "CN=Subject");
        ...
        // 3) 签名算法 (此处以 Dilithium OID 为例)
//        AlgorithmIdentifier sigAlg = new AlgorithmIdentifier(X9ObjectIdentifiers.ecdsa_with_SHA256);
        AlgorithmIdentifier sigAlg = new AlgorithmIdentifier(DILITHIUM_OID);
        // 4) 有效期
        Date notBefore;
...
//        SubjectPublicKeyInfo spki = new SubjectPublicKeyInfo(pkAlg, pubBitString);
        ...
        //重构Dilithium
        AlgorithmIdentifier pkAlg = new AlgorithmIdentifier(DILITHIUM_OID);
        SubjectPublicKeyInfo spki = new SubjectPublicKeyInfo(pkAlg, pubBitString);
        ...
        if (extension != null && extension.length > 0) {
            tbsGen.setExtensions(Extensions.getInstance(extension));
        }
...
//        byte[] sigContent = sigField; // 直接整个数组
        // 直接将 Dilithium 签名字节数组 (3293 bytes) 包装为 BIT STRING
    ...
    //验证CI的自签名公钥证书
    public static void verifySelfSignedCICert_Dilithium3(
            X509Certificate ciCert) throws Exception {

        ciCert.checkValidity();

        ciCert.verify(ciCert.getPublicKey(), "BCPQC");
    }
