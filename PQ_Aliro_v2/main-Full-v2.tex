...
the ECC baseline of 1.2 KB---while a \emph{key\_slot} caching optimisation reduces this to
...
\]
The transcript hash $\mathit{trans} = \msh(pk_e \| c_e)$ in our protocol is modelled as a collision-resistant hash function; the security reduction in Section~\ref{sec:proof} (Game 1) relies on this property.
\end{defn}
...
Key derivation employs the HMAC-based Key Derivation Function ($\hkdf$)~\cite{RFC5869}.
$\hkdf$ follows the extract-then-expand paradigm, using HMAC~\cite{DBLP:journals/rfc/rfc2104} as its building block.
...
A pseudorandom function $\mathsf{PRF}: \mathcal{K} \times \mathcal{L} \to \{0,1\}^\lambda$ is secure if for every PPT adversary $\mathcal{A}$,
\[
\mathsf{Adv}_{\mathsf{PRF}}^{\mathsf{PRF}\text{-}\mathsf{sec}}(\mathcal{A}, \lambda) = \left| \Pr\left[ k \gets \mathcal{K} : \mathcal{A}^{\mathsf{PRF}(k,\cdot)} = 1 \right] - \Pr\left[ \mathcal{A}^{R(\cdot)} = 1 \right] \right| \leq \negli(\lambda),
\]
where $R: \mathcal{L} \to \{0,1\}^\lambda$ is a uniformly random function.
\end{defn}

In our protocol, $\mathsf{HKDF.Extract}$ and $\mathsf{HKDF.Expand}$~\cite{RFC5869} are each modelled as a PRF instance; the input domain $\mathcal{L}$ corresponds to the transcript space, and the output is the session key material.
...
(including \texttt{trans\_id}, \texttt{reader\_id}, and UD noise) using
...
AUTH0 Cmd    & R$\to$UD & \texttt{0x80} &
    $pk_e$ (1\,184\,B), \textit{trans\_id} (16\,B), \textit{reader\_id} (32\,B) &
    200\,B \\
AUTH0 Resp   & UD$\to$R & --- &
    KEM ciphertext (1\,088\,B), UD noise (32\,B) &
    240\,B \\
LOADCERT Cmd & R$\to$UD & \texttt{0xD1} &
    Profile0000 cert ($\approx$5\,500\,B): $pk_{R}$ (1\,952\,B), $\sigma$ (3\,309\,B), DN ($\approx$200\,B) &
    255\,B \\
...
To realise tracking resilience, any information that could serve as a stable identifier for a User Device (e.g., its identity) must be confidentiality-protected while in transit.
...
declared with the Android manifest attribute \texttt{android:process=":pqc\_signer"}.
...
This is distinct from \texttt{android:isolatedProcess="true"}, which would
assign a distinct UID and deny file-system access; the named-process approach
is used here because \texttt{PQCSignatureService} requires access to
\texttt{SharedPreferences} for encrypted key retrieval.
...
held in the \texttt{:pqc\_signer} address space.
...
\mathrm{SHAKE256\text{-}512}(\mathit{AUTH0\_cmd} \mathbin{\|} \mathit{AUTH0\_resp})$~\cite{NIST:FIPS202}
...
The classical ECC Aliro flow (Curve P-256) transfers approximately 1.2\,KB (comprising an ephemeral ECDH public key 65\,B, an ECDSA signature $\approx$72\,B, a P-256 certificate $\approx$1\,000\,B, and framing overhead across $\approx$8\,APDUs, totalling $\approx$1.2\,KB)
...
Per-APDU round-trip overhead---including ISO/IEC~14443-4~\cite{ISOIEC14443} Frame Waiting Time (FWT) and Reader polling intervals---is not included;
...
key-encapsulation and signature primitives, and its $\approx$16.4\,KB
...
($\approx$0.7\,s) and, assuming the theoretical latency estimates hold on real hardware, would be suitable for high-assurance installations such
...
The \texttt{key\_slot} optimisation pre-provisions Reader certificates on the User Device.
...
the \texttt{transaction\_identifier} to mitigate this leakage.
...
knowledge. The \texttt{transaction\_identifier} provides replay protection but
