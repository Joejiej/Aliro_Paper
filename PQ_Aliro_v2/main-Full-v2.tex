...
We prove, within a Bellare--Rogaway-style symbolic model, that PQ-Aliro achieves mutual
authentication, forward secrecy, and identity privacy under the assumed hardness of the
ML-KEM and ML-DSA problems.
...
We conduct an exhaustive byte-accurate analysis of communication overhead for all nine PQC parameter combinations (Dilithium$\{2,3,5\} \times $Kyber$\{512,768,1024\}$). The default ML-DSA-65 $\times$ ML-KEM-768 configuration incurs $\approx$16.4~KB and $\approx$75 APDUs per transaction; the lightest ML-DSA-44 $\times$ ML-KEM-512 configuration reduces this to $\approx$11.9~KB and $\approx$55 APDUs. We additionally show that a \emph{key-slot} optimisation---pre-caching the reader certificate on the user device---eliminates the LOADCERT round and saves $\approx$5--7~KB.

\item \textbf{Formal security analysis.}
Building on the Bellare--Rogaway security model~\cite{C:BelRog93} and the provable security of ML-KEM~\cite{EuroSP:BDKLLSSSS18} and ML-DSA~\cite{TCHES:DKLLSSS18}, we provide a formal security proof establishing that PQ-Aliro achieves mutual authentication, forward secrecy, and reader-identity privacy under the module learning-with-errors (MLWE) and module short-integer-solution (MSIS) hardness assumptions in the Bellare--Rogaway model~\cite{C:BelRog93} (see \S\,6--7 for the full model and proof).

\item \textbf{Engineering insights for constrained Android HCE.}
We document critical implementation findings: isolating Dilithium private-key operations inside an \texttt{isolatedProcess} Service to prevent cross-process key leakage, and dispatching ML-DSA signing asynchronously to a background coroutine to avoid ANR deadlocks on the HCE main thread. These lessons are expected to generalise to other lattice-based protocols deployed over bandwidth-constrained ISO-DEP transports.

\end{enumerate}

\noindent
The remainder of this paper is organised as follows.
Section~2 surveys related work on PQC integration in constrained communication protocols and NFC/BLE access control.
Section~3 provides background on the Aliro~1.0 protocol and the ML-KEM/ML-DSA algorithms.
Section~4 introduces formal definitions and notation used throughout the security analysis.
Section~5 presents the PQ-Aliro protocol design, APDU message formats, and trust framework.
...
Section~9 reports the communication-overhead evaluation and NFC timing measurements.
...
% -----------------------------------------------------------------------
% §4 Definitions and Components (from main-Full.tex)
% -----------------------------------------------------------------------
...
\textbf{Common initialisation} (for all three experiments):
\begin{enumerate}[leftmargin=2em,label=\arabic*.]
  \item $pp_{\mathsf{AKE}} \gets \mathsf{AKE.Setup}(1^\lambda)$.\quad
        For each $i\in[\mu]$: $(pk_i,sk_i)\gets\mathsf{AKE.Gen}(pp_{\mathsf{AKE}},i)$.
  \item Challenger $\mathcal{C}$ samples $b\gets\!\$\,\{0,1\}$ and gives
        $\bigl(pp_{\mathsf{AKE}},\,\mathsf{PKList}{:=}\{pk_i\}_{i\in[\mu]}\bigr)$ to $\mathcal{A}$.
  \item $\mathcal{A}$ adaptively issues oracle queries from
        $\{\mathsf{Send},\mathsf{Respond},\mathsf{Corrupt},\mathsf{RegisterCorrupt},
          \mathsf{SessionKeyReveal},\mathsf{TestKey},\mathsf{TestPrivacy}\}$
        (at most one of $\mathsf{TestKey}$/$\mathsf{TestPrivacy}$, not both).
  \item $\mathcal{A}$ outputs $b^*$; $\mathcal{C}$ returns $1$ iff $b^*=b$.
\end{enumerate}

\medskip
\begin{tabular}{@{}p{0.30\textwidth}@{\quad}p{0.30\textwidth}@{\quad}p{0.30\textwidth}@{}}
\underline{$\mathsf{Exp}^{\mathsf{IND}}_{\mathsf{AKE},\mu,\ell,\mathcal{A}}$}
&
\underline{$\mathsf{Exp}^{\mathsf{Privacy}}_{\mathsf{AKE},\mu,\ell,\mathcal{A}}$}
&
\underline{$\mathsf{Exp}^{\mathsf{AUTH}}_{\mathsf{AKE},\mu,\ell,\mathcal{A}}$}
\\[2pt]
\textit{Active oracle:} $\mathsf{TestKey}$.
Via $\mathsf{TestKey}(i^*,s^*)$,
$\mathcal{A}$ receives either the real
session key $k^{s^*}_{i^*}$ or a
uniform random key $k_1$.
$\mathcal{A}$ wins by guessing $b$.
Forbidden: trivial attacks TA1--TA3.
$\mathsf{Adv}^{\mathsf{IND}}:=\bigl|\Pr[\cdots\Rightarrow1]-\tfrac{1}{2}\bigr|$.
&
\textit{Active oracle:} $\mathsf{TestPrivacy}$.
Via $\mathsf{TestPrivacy}(i_0,j_0,i_1,j_1)$,
$\mathcal{A}$ receives a transcript of
either the $(i_0,j_0)$-session or the
$(i_1,j_1)$-session.
$\mathcal{A}$ wins by guessing $b$.
Forbidden: trivial attacks TA4--TA6.
$\mathsf{Adv}^{\mathsf{Privacy}}:=\bigl|\Pr[\cdots\Rightarrow1]-\tfrac{1}{2}\bigr|$.
&
\textit{Event:} $\mathsf{WinAuth}$.
Checked at the end of
$\mathsf{Exp}^{\mathsf{IND}}$ or
$\mathsf{Exp}^{\mathsf{Privacy}}$.
$\mathsf{WinAuth}$ occurs if some
$\pi^s_i$ accepts yet has no
matching partner $\pi^t_j$,
or two distinct oracles both
accept the same session.
$\mathsf{Adv}^{\mathsf{AUTH}}:=\Pr[\mathsf{WinAuth}]$.
\\
\end{tabular}
\end{minipage}%
}
\caption{Security experiments for the PQ-Aliro AKE scheme.
         Trivial attacks TA1--TA6 are listed in Table~\ref{tab:trivial-attacks}.}
\label{fig:security-experiments}
\end{figure}
...
% -----------------------------------------------------------------------
% §7 Security Proof (from main-Full.tex)
% -----------------------------------------------------------------------
